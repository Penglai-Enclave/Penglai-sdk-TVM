...
/* Define: TIMER_RES_DIVIDER
	...
	*/
    ...
    #define read_csr(reg) ({ unsigned long __tmp; \
       ...
       __tmp; })
	#define read_cycle() ({ unsigned long __tmp; \
		asm volatile ("rdcycle %0":"=r"(__tmp)); \
		__tmp; })
    #define GETMYTIME(_t) (*_t=read_csr(cycle))
    // #define GETMYTIME(_t) (*_t=read_cycle())
	#define MYTIMEDIFF(fin,ini) ((fin)-(ini))
	#define TIMER_RES_DIVIDER 1
	#define SAMPLE_TIME_IMPLEMENTATION 1
#endif
...
*/
#if PERFORMANCE_BREAKDOWN
CORE_TICKS get_time(CORE_TICKS start, CORE_TICKS stop) {
	CORE_TICKS elapsed;
	if (start == 0 && stop == 0) {
		elapsed=(CORE_TICKS)(MYTIMEDIFF(stop_time_val, start_time_val));
	} else {
		elapsed = stop - start;
	}
	return elapsed;
