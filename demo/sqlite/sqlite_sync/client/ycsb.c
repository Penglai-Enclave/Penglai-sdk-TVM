...
int generate_table(const char *prefix, int record_cnt, int field_cnt, int field_sz, void*sql, unsigned long shm_size, unsigned long handler)
{
    // char key_name[101];
    ...
    LOG_DEBUG("%s\n", sql);    
    call_arg.req_vaddr = sql;
    call_arg.req_size = shm_size;
    call_arg.req_arg = OTHER;
    ret = call_enclave(handler, &call_arg);
...
void performance_test(int record_cnt, int operation_cnt, const char* prefix, int read_proportion, int update_proportion, int insert_proportion, int scan_proportion, void*sql, unsigned long shm_size, unsigned long handler)
{
