...
int main(int argc, char** argv)
{
  ...
  if (argc !=4)
  {
    printf("Usage: ./host ipc_manager server client\n");
    return -1;
  }
  int needed_argc = 1;
...
callee:
    ...
    if(!elf_valid(callee_enclaveFile)){
      printf("host: error when initializing async sqlite-server\n");
      goto out;
    }
    ...
    if(PLenclave_create(callee_enclave, callee_enclaveFile, callee_params) < 0){
      printf("host: failed to create async sqlite-server, may have been created\n");
    }
caller:
  ...
  debug_printf("async sqlite test finished\n");
out:
  if(ipc_manager_elf){
