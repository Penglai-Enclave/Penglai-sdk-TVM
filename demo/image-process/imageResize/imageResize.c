...
int sod_resize_main(unsigned long arg0, void* vaddr, unsigned long size)
{
	...
	sod_copy_image_inplace(rz, vaddr);
	sod_free_image(rz);
	//sod_free_image(rot);
...
int EAPP_ENTRY main(){
 ...
 // eapp_print("server1: read req_vaddr:0x%lx\n", sum);
  ...
  call_arg.resp_val = 3;
  eapp_debug_print("ImageResize is exiting with retval:0x%lx\n", 3);
  SERVER_RETURN(&call_arg);
}
