...
#define ASSERT( predicate )                                                   \
    ...
    {                                                                         \
        if( ! ( predicate ) )                                                 \
        {                                                                     \
            eapp_print( "\tassertion failed at %s:%d - '%s'\r\n",         \
...
#define ASSERT_STATUS( actual, expected )                                     \
    ...
    {                                                                         \
        if( ( actual ) != ( expected ) )                                      \
        {                                                                     \
            eapp_print( "assertion failed at %s:%d - "                  \
                    "actual:%d expected:%d\r\n", __FILE__, __LINE__,  \
...
// int main( void )
// {
//     eapp_print( "MBEDTLS_PSA_CRYPTO_C and/or MBEDTLS_AES_C and/or "
...
static void cipher_examples( void )
{
    ...
    eapp_print( "cipher encrypt/decrypt AES CBC no padding:\r\n" );
    status = cipher_example_encrypt_decrypt_aes_cbc_nopad_1_block( );
    if( status == PSA_SUCCESS )
        eapp_print( "\tsuccess!\r\n" );

    eapp_print( "cipher encrypt/decrypt AES CBC PKCS7 multipart:\r\n" );
    status = cipher_example_encrypt_decrypt_aes_cbc_pkcs7_multi( );
    if( status == PSA_SUCCESS )
        eapp_print( "\tsuccess!\r\n" );

    eapp_print( "cipher encrypt/decrypt AES CTR multipart:\r\n" );
    status = cipher_example_encrypt_decrypt_aes_ctr_multi( );
    if( status == PSA_SUCCESS )
        eapp_print( "\tsuccess!\r\n" );
}

int EAPP_ENTRY main( void )
{
    unsigned long * args;
    EAPP_RESERVE_REG;
