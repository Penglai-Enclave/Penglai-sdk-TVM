...
// int main( void )
// {
//     eapp_print( "MBEDTLS_SHA256_C and/or MBEDTLS_MD_C and/or "
                              ...
                              const char *output_file_name )
{
    ...
    eapp_print("key_file %lx, out_file_name %s\n", key_file, output_file_name);
                                          ...
                                          psa_key_id_t *master_key )
{
    ...
    {
        eapp_print( "Key file too large (max: %u).\n",
                               ...
                               psa_key_id_t wrapping_key )
{
    ...
    {
        eapp_print( "Input file too large.\n" );
        status = DEMO_ERROR;
        goto exit;
    }
#endif
                                 ...
                                 psa_key_id_t wrapping_key )
{
    ...
    {
        eapp_print( "The input does not start with a valid magic header.\n" );
        status = DEMO_ERROR;
        goto exit;
    }
    if( header.ad_size != sizeof( header ) )
    {
        eapp_print( "The header size is not correct.\n" );
        status = DEMO_ERROR;
        goto exit;
    }
    ciphertext_size =
    ...
    {
        eapp_print( "Extra garbage after ciphertext\n" );
        status = DEMO_ERROR;
        goto exit;
    }
    SYS_CHECK( fclose( input_file ) == 0 );
    ...
    {
        eapp_print( "Incorrect payload size in the header.\n" );
        status = DEMO_ERROR;
        goto exit;
    }
...
{
    eapp_print( "Usage: key_ladder_demo MODE [OPTION=VALUE]...\n" );
    eapp_print( "Demonstrate the usage of a key derivation ladder.\n" );
    eapp_print( "\n" );
    eapp_print( "Modes:\n" );
    eapp_print( "  generate  Generate the master key\n" );
    eapp_print( "  save      Save the derived key\n" );
    eapp_print( "  unwrap    Unwrap (decrypt) input with the derived key\n" );
    eapp_print( "  wrap      Wrap (encrypt) input with the derived key\n" );
    eapp_print( "\n" );
    eapp_print( "Options:\n" );
    eapp_print( "  input=FILENAME    Input file (required for wrap/unwrap)\n" );
    eapp_print( "  master=FILENAME   File containing the master key (default: master.key)\n" );
    eapp_print( "  output=FILENAME   Output file (required for save/wrap/unwrap)\n" );
    eapp_print( "  label=TEXT        Label for the key derivation.\n" );
    eapp_print( "                    This may be repeated multiple times.\n" );
    eapp_print( "                    To get the same key, you must use the same master key\n" );
    eapp_print( "                    and the same sequence of labels.\n" );
}
...
int EAPP_ENTRY main(  )
{
    ...
    argc = 3;
    argv = malloc(3*8);
    eapp_print("after malloc\n");
    argv[0] = "./psa_key_ladder";
    argv[1] = "generate";
    argv[2] = "master=master.key";
    eapp_print("after passing the arguments\n");
    ...
    {
        eapp_print( "Required argument missing: input\n" );
        EAPP_RETURN( DEMO_ERROR );
    }
    if( output_file_name == NULL &&
        ( mode == MODE_SAVE || mode == MODE_WRAP || mode == MODE_UNWRAP ) )
    {
        eapp_print( "Required argument missing: output\n" );
        EAPP_RETURN( DEMO_ERROR );
    }
    ...
    eapp_print("GENERATE is successful\n");
    mode = MODE_WRAP;
    input_file_name = "/input.txt";
    output_file_name = "/hello_world.wrap";
    ladder[0] = "hello";
    ladder[1] = "world";
    ladder_depth = 2;

    FILE* infile = fopen("/input.txt", "wb");
    if(infile == NULL)
    {
        eapp_print("fopen error");
        EAPP_RETURN(-1);
    }

    size_t write_ret = fwrite("Here is some input. See it wrapped.", 1, sizeof("Here is some input. See it wrapped."), infile);
    if(write_ret != sizeof("Here is some input. See it wrapped."))
    {
        eapp_print("fwrite error");
        EAPP_RETURN(-1);
    }
    fclose(infile);
    ...
    eapp_print("WRAP is successful\n");

    mode = MODE_UNWRAP;
    input_file_name = "/hello_world.wrap";
    output_file_name = "/hello_world.txt";
    ladder[0] = "hello";
    ladder[1] = "world";
    ladder_depth = 2;
    ...
    eapp_print("UNWRAP is successful\n");

    char buf1[200] = {0};
    FILE* cmp_file1 = NULL;
    cmp_file1 = fopen("/input.txt","rb");
    if (cmp_file1 == NULL)
    {
        eapp_print("fopen failed\n");
    }
    memset(buf1,0,200);
    int res1 = fread(buf1, 1, 10, cmp_file1);
    if(res1 == 0){
        eapp_print("fread failed\n");
    }
    eapp_print("input.txt content: %lx\n", *(long *)buf1);

    char buf2[200] = {0};
    FILE* cmp_file2 = fopen("/hello_world.txt","rb");
    if (cmp_file2 == NULL)
    {
        eapp_print("fopen failed\n");
    }
    memset(buf2,0,200);
    int res2 = fread(buf2, 1, 10, cmp_file2);
    if(res2 == 0){
        eapp_print("fread failed\n");
    }
    eapp_print("hello_world.txt content: %lx\n", *(long *)buf2);

    EAPP_RETURN( status == PSA_SUCCESS ?
            EXIT_SUCCESS :
            EXIT_FAILURE );

usage_failure:
    usage( );
    EAPP_RETURN( EXIT_FAILURE );
}
