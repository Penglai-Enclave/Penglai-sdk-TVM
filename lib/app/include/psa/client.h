...
/**
 * An PSA call type indicates the corresponding psa function.
 */
#define PSA_PS_SET                (0)

#define PSA_PS_GET                (1)

#define PSA_PS_GET_INFO           (2)

#define PSA_PS_REMOVE             (3)

#define PSA_PS_CREATE             (4)

#define PSA_PS_SET_EXTENDED       (5)

#define PSA_PS_GET_SUPPORT        (6)
...
typedef struct psa_invec_offset {
    size_t offset;           /*!< the start address offset of the memory buffer */
    size_t len;                 /*!< the size in bytes                      */
} psa_invec_offset;
...
typedef struct psa_callee_outvec {
    void *base;                 /*!< the start address of the memory buffer */
    size_t* len_addr;           /*!< the address of parameter len in out_vec*/
} psa_callee_outvec;

typedef struct psa_outvec_offset {
    size_t offset;                 /*!< the start address offset of the memory buffer */
    size_t len;                 /*!< the size in bytes                      */
} psa_outvec_offset;
...
psa_status_t psa_call_stub(int32_t type,
                      const psa_invec *in_vec,
                      size_t in_len,
                      psa_outvec *out_vec,
                      size_t out_len);
